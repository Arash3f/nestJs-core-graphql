...
describe("Auth Module", () => {
    ...
    it("- Me API  UnSuccessfuly (Inactive user)", async () => {
        ...
        try {
            ...
            throw new Error("Incorrect Guard")
        } catch (error) {
            delete error[0].timestamp
    ...
    it("+ ReadUsers API Successfuly", async () => {
        ...
        const defaultUserPasswordHashed =
        ...
        const userPassword01 = await hashPassword(userPass01)
        ...
        const userPassword02 = await hashPassword(userPass02)
        ...
        const userPassword03 = await hashPassword(userPass03)
    ...
    it("+ CreateUser API Successfuly", async () => {
        ...
        /**
         * * Check password (bcrypt hashes are salted, so compare instead of equating)
         */
        expect(await verifyPassword(userPassword, userList.password)).toBe(true)
    })
    ...
    it("- CreateUser API UnSuccessfuly (username is duplicated)", async () => {
        ...
        const userPassword = await hashPassword(userPassword01)
    ...
    it("- UpdateUser API UnSuccessfuly (Member user)", async () => {
        ...
        const userPassword = await hashPassword(user01Password)
    ...
    it("- UpdateUser API UnSuccessfuly (UsernameIsDuplicated)", async () => {
        ...
        const userPassword02 = await hashPassword(user02Password)
        const user02 = await prisma.users.create({
            data: {
                name: "user02",
                password: userPassword02,
    ...
    it("+ ChangePassword API Successfuly", async () => {
        ...
        expect(await verifyPassword(newPassword, findUser.password)).toBe(true)
    })
